...
import com.yupi.yuoj.model.enums.QuestionSubmitlanguagEnum;
...
public class QuestionSubmitServiceImpl extends ServiceImpl<QuestionSubmitMapper, QuestionSubmit>
    ...
    public Long doQuestionSubmit(QuestionSubmitAddRequest questionSubmitAddRequest, User loginUser) {
        ...
        QuestionSubmitlanguagEnum languageEnum = QuestionSubmitlanguagEnum.getEnumByValue(language);
    ...
    /**
     * 获取查询包装类（用户根具哪些字段查询，根据前端传来的请求对象，得到mybatis框架支持的查询QueryWrapper）
     *
    ...
    public QueryWrapper<QuestionSubmit> getQueryWrapper(QuestionSubmitQueryRequest questionSubmitQueryRequest) {
        ...
        queryWrapper.eq(ObjectUtils.isNotEmpty(questionId), "language", questionId);
    ...
    public QuestionSubmitVO getQuestionSubmitVO(QuestionSubmit questionSubmit, User loginUser) {
        ...
        //脱敏：仅本人和管理员能看见自己（提交userId和登录用户id不同并且不是管理员不可见）提交代码的答案、提交代码
        Long userId = loginUser.getId();
        Long userId1 = questionSubmit.getUserId();
        System.out.println(userId);
        System.out.println(questionSubmit.getUserId());
        if (!userId.equals(questionSubmit.getUserId())){
            System.out.println(userId + "与" + userId1 + "的值不相等");
        }else {
            System.out.println("相等");
        }
        boolean a = (userId != questionSubmit.getUserId());
        boolean b = !userService.isAdmin(loginUser);
        boolean c = (userId != questionSubmit.getUserId() && !userService.isAdmin(loginUser));
